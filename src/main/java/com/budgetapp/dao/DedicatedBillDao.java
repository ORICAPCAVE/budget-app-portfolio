...
public class DedicatedBillDao {
    ...
    public List<DedicatedBill> findAll() {
        ...
        String sql = "SELECT id, name, amount, recurrence, category FROM dedicated_bill";
    ...
    public void save(DedicatedBill bill) {
        String sql = """
                INSERT INTO dedicated_bill(name, amount, recurrence, category)
                VALUES (?, ?, ?, ?)
                """;
        ...
        try (Connection conn = db.getConnection();
            ...
            ps.setString(4, bill.getCategory().name());
    ...
    public void update(DedicatedBill bill) {
        String sql = """
            ...
            SET name = ?, amount = ?, recurrence = ?, category = ?
            WHERE id = ?
            """;
